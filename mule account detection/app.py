...
st.markdown(
    ...
    .hero { position:relative; isolation:isolate; overflow:hidden; display:grid; grid-template-columns:minmax(0,1.3fr) minmax(245px,.7fr); gap:2rem; background:#10150d; border:1px solid #2d3926; border-radius:24px; padding:clamp(1.8rem,4vw,3.6rem); margin:.5rem 0 1.5rem; box-shadow:0 22px 45px rgba(12,16,8,.16); }
    .hero::before { content:''; position:absolute; z-index:-1; width:360px; height:360px; right:-100px; top:-140px; border-radius:50%; background:radial-gradient(circle, rgba(183,237,27,.42), rgba(183,237,27,0) 68%); }
    .hero::after { content:''; position:absolute; z-index:-1; inset:0; opacity:.45; background-image:linear-gradient(90deg, rgba(183,237,27,.08) 1px, transparent 1px), linear-gradient(rgba(183,237,27,.08) 1px, transparent 1px); background-size:34px 34px; mask-image:linear-gradient(90deg, #000, transparent 86%); }
    .hero-kicker { display:flex; align-items:center; gap:.48rem; color:#b7ed1b; font-size:.73rem; letter-spacing:.14em; font-weight:800; text-transform:uppercase; }.hero-kicker::before { content:'✦'; font-size:.95rem; }
    .hero-title { color:#fff; font-size:clamp(2.25rem,4.6vw,4.6rem); line-height:.94; letter-spacing:-.065em; font-weight:850; margin:.85rem 0; max-width:720px; }
    .hero-copy { color:#d1d8c8; max-width:560px; font-size:1rem; line-height:1.6; }
    .status-pill { display:inline-flex; align-items:center; gap:.42rem; padding:.45rem .75rem; border-radius:999px; color:#e1f9a5; background:rgba(183,237,27,.10); border:1px solid rgba(183,237,27,.35); font-size:.77rem; font-weight:700; }
    .status-pill::before { content:''; width:7px; height:7px; border-radius:50%; background:#b7ed1b; box-shadow:0 0 12px #b7ed1b; }
    .hero-visual { position:relative; min-height:210px; display:flex; align-items:center; justify-content:center; }.hero-orb { position:relative; width:158px; height:158px; display:grid; place-items:center; border-radius:50%; background:radial-gradient(circle at 34% 28%, #d9ff72, #9ed51b 43%, #39550d 72%); box-shadow:0 0 0 15px rgba(183,237,27,.08), 0 0 0 34px rgba(183,237,27,.045), 0 22px 45px rgba(0,0,0,.35); color:#122009; font-size:2.35rem; font-weight:900; }.hero-orb small { position:absolute; bottom:38px; font-size:.65rem; letter-spacing:.12em; }
    .hero-card { position:absolute; min-width:138px; padding:.78rem .85rem; border:1px solid rgba(224,241,198,.20); border-radius:14px; background:rgba(17,24,14,.78); backdrop-filter:blur(9px); box-shadow:0 10px 25px rgba(0,0,0,.22); color:#fff; }.hero-card strong { display:block; color:#c9fa43; font-size:1rem; }.hero-card span { display:block; margin-top:.18rem; color:#bbc7b0; font-size:.68rem; letter-spacing:.05em; text-transform:uppercase; }.hero-card.one { top:10px; right:0; animation:float-one 4.8s ease-in-out infinite; }.hero-card.two { bottom:4px; left:0; animation:float-two 5.5s ease-in-out infinite; }
    @keyframes float-one { 50% { transform:translateY(-8px); } } @keyframes float-two { 50% { transform:translateY(8px); } }
    @media (max-width: 700px) { .intro { border-radius: 18px; } .intro-nav { margin-bottom: 3rem; } .intro-live { display:none; } .hero { grid-template-columns:1fr; } .hero-visual { min-height:185px; } [data-baseweb="tab-list"] { border-radius:18px; } }
...
st.markdown(
    ...
    <section class="hero">
      <div>
        <div class="hero-kicker">Live investigation workspace</div>
        <div class="hero-title">MAKE THE<br><span style="color:#b7ed1b;">INVISIBLE</span> CLEAR.</div>
        <div class="hero-copy">Turn complex account behaviour into clear, defensible next steps. Review signals, surface risk, and focus investigators where they matter most.</div>
        <div style="margin-top:1.3rem;"><span class="status-pill">Model intelligence online</span></div>
      </div>
      <div class="hero-visual" aria-label="MuleGuard risk intelligence status">
        <div class="hero-orb">AI<small>RISK ENGINE</small></div>
        <div class="hero-card one"><strong>54</strong><span>Risk signals mapped</span></div>
        <div class="hero-card two"><strong>Ready</strong><span>Secure analysis</span></div>
      </div>
    </section>
    """,
    unsafe_allow_html=True,
